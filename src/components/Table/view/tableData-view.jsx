...
import { getReview, deleteReview,} from 'src/http';
...
export default function TableData() {
  ...
  const fetchData = async () => {
    const data = await getReview({ search, page, limit: rowsPerPage, consultant: param.id })
    setReviewData(data)
  }
  useEffect(() => {
    fetchData()
        ...
        <Scrollbar>
          <TableContainer sx={{ overflow: 'unset' }}>
            <Table sx={{ minWidth: 800 }}>
              ...
              <TableBody>
                {
                  reviewData?.result?.length ?
                    reviewData?.result?.map((row) => (
                      <UserTableRow
                        key={row._id}
                ...
                {rowsPerPage > reviewData?.result?.length ? <TableEmptyRows
                  height={77}
                  emptyRows={reviewData?.result?.length > 2 ? 1 : 2}
                /> : null}
              </TableBody>
            </Table>
          </TableContainer>
